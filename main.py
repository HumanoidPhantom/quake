...
#main_id = hashlib(sha1(.encode()).hexdigest())
				...
				if class_method == 0:
					...
					if node_id in list_neighbours:
						...
						if code == 1:
							...
							print ('Excellent!!!!', dic_neighbours)
							list_neighbours.append(node_id)
							time.sleep(3)

							th_newNode = threading.Thread(target = updateLun, args =(node_id,info))
							th_newNode.start()
						elif code == 0:
...
def listen(port):
	...
	while th_main.isAlive():
		...
		for j in range(i,i+1):
			...
			th[j] = threading.Thread(target = main, args=(list_sockets[j],list_addr[j]))
			th[j].start()
...
th_main = threading.Thread(target=listen, args = (port,) )
th_init = threading.Thread(target = checkStatus, args= ())
th_main.start()
th_init.start()
th_main.join()
th_init.join()
